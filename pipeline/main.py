...
# Run evaluation based on dataset and method
dataset = config["configs"]["eval_config"]["dataset"]
method = config["configs"]["pipeline_config"]["method"]

if dataset == 'mquake_remastered':
    if method == 'gwalk':
        from pipeline.gwalk.eval import eval_gwalk
        raw_results, processed_results = eval_gwalk(config)
    elif method == 'mello':
        from pipeline.mello.eval import eval_mello
        raw_results, processed_results = eval_mello(config)
    else:
        logger.error(f"Unknown method passed: {method}")
