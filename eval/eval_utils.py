...
def mquake_accuracy(results):
    """MQuAKE-Remastered accuracy metric.

    For MQuAKE, accuracy is tracked per-type (edited/unedited) inside the
    MQuAKE_Remastered dataset class itself. This metric is a placeholder
    that returns the result_summary string already computed by the dataset.
    The actual metric computation happens in the eval loop and is passed
    through via the processed_results dict.
    """
    return "See processed_results for type-based accuracy breakdown."
...
METRIC_FUNCTIONS = {
    ...
    "mquake_accuracy": mquake_accuracy,
}